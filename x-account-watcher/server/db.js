...
// Only load sqlite3 when running locally (not on Vercel serverless)
if (process.env.VERCEL !== '1') {
  try {
    ...
    console.warn('[DB] SQLite3 local binding skipped:', err.message);
  }
}
...
async function initDb() {
  if (supabase) {
    console.log('[DB] Supabase cloud instance ready.');
    return;
  }
...
// Database Helper Methods with Supabase Priority
const dbAsync = {
  async all(sql, params = []) {
    if (supabase) {
      try {
        ...
        console.warn('[Supabase Select Error]:', err.message);
      }
    }
    ...
    if (supabase) {
      try {
        if (sql.includes('INSERT INTO accounts')) {
          ...
          return { changes: 1 };
        } else if (sql.includes('UPDATE accounts SET active = 0')) {
          const [id] = params;
